...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['size'] > 0) {
        $target_dir = "../../assets/img/avartar/$id/"; // Thư mục lưu avatar
    ...
    if ($avatar !== "") {
        $sql = "UPDATE nguoidung SET USERNAME=?, HO=?, TEN=?, EMAIL=?, SDT=?, AVATAR=? WHERE IDNGUOIDUNG=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssi", $username, $ho, $ten, $email, $sdt, $avatar, $id);
    } else {
        $sql = "UPDATE nguoidung SET USERNAME=?, HO=?, TEN=?, EMAIL=?, SDT=? WHERE IDNGUOIDUNG=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $username, $ho, $ten, $email, $sdt, $id);
    }
